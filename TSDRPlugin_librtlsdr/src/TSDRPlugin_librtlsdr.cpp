...
EXTERNC TSDRPLUGIN_API int __stdcall tsdrplugin_init(const char * params) {
	...
	try {
		...
		if (r < 0) {
			fprintf(stderr, "Can't set sample rate.\n");
			throw;
		}
		...
		if (r < 0) {
			fprintf(stderr, "Can't set bandwidth.\n");
			throw;
		}
		...
		if (r < 0) {
			fprintf(stderr, "Can't set center frequency.\n");
			throw;
		}
		...
		if (0 == req_gain) {
			...
			if (r < 0) {
				fprintf(stderr, "Can't set auto gain mode.\n");
				throw;
			}
		} else {
			/* Enable manual gain */
			...
			r = rtlsdr_set_tuner_gain_mode(dev, 1);
			if (r < 0) {
				fprintf(stderr, "Can't set manual gain mode.\n");
				throw;
			}
...
EXTERNC TSDRPLUGIN_API int __stdcall tsdrplugin_readasync(tsdrplugin_readasync_function cb, void *ctx) {
	...
	try {
		...
		if (r < 0) {
			fprintf(stderr, "Can't reset buffer.\n");
			throw;
		}
